...
def test_xp_and_avatar_update_flow(client):
    ...
    # 1. Register user
    register_response = client.post("/register", json={"name": "GamificationUser", "password": "StrongPass1!"})
    ...
    login_response = client.post("/login", json={"name": "GamificationUser", "password": "StrongPass1!"})
...
def test_multiple_xp_updates_accumulate(client):
    ...
    register_response = client.post("/register", json={"name": "XPAccumUser", "password": "StrongPass1!"})
    token = register_response.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
    ...
    login_response = client.post("/login", json={"name": "XPAccumUser", "password": "StrongPass1!"})
    user = login_response.json()["user"]
...
def test_avatar_persistence_across_sessions(client):
    ...
    register_response = client.post("/register", json={"name": "PersistAvatar", "password": "StrongPass1!"})
    ...
    login_response = client.post("/login", json={"name": "PersistAvatar", "password": "StrongPass1!"})
    user = login_response.json()["user"]
